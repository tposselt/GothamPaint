...
public static class Layout
{
    ...
    public delegate void OnExportCanvas();
    public static OnExportCanvas ExportCallback = delegate { };
    ...
    public static unsafe void Sidebar()
    {
        ...
        }))
        {
            ...
            using (Clay.Element(Clay.Id("Footer"), new()
            {
                floating = new()
                {
                    offset = new() { x = sidebarWidth, y = 0 },
                    attachTo = Clay_FloatingAttachToElement.CLAY_ATTACH_TO_PARENT,
                    attachPoints = new()
                    {
                        element = Clay_FloatingAttachPointType.CLAY_ATTACH_POINT_LEFT_BOTTOM,
                        parent = Clay_FloatingAttachPointType.CLAY_ATTACH_POINT_LEFT_BOTTOM,
                    }
                },
                layout = new()
                {
                    sizing = new Clay_Sizing(Clay_SizingAxis.Grow(), Clay_SizingAxis.Fixed(75)),
                    padding = Clay_Padding.All(5),
                    layoutDirection = Clay_LayoutDirection.CLAY_LEFT_TO_RIGHT,
                    childAlignment = new() { x = Clay_LayoutAlignmentX.CLAY_ALIGN_X_LEFT, y = Clay_LayoutAlignmentY.CLAY_ALIGN_Y_BOTTOM },
                    childGap = 5,
                }
            }))
            {
                using (Clay.Element(Clay.Id("ResizePopupButton"), new()
                {
                    backgroundColor = BackgroundColor,
                    layout = new()
                    {
                ...
                using (Clay.Element(Clay.Id("ExportImageButton"), new()
                {
                    backgroundColor = BackgroundColor,
                    layout = new()
                    {
            ...
            }))
            {
                ...
                using (Clay.Element(Clay.Id("Filler"), new()
                {
                    backgroundColor = BackgroundColor,
                    layout = new()
                    {

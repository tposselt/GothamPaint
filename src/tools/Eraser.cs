...
public class Eraser(Texture2D canvasTexture) : PaintTool(canvasTexture)
{
    ...
    public override void Draw(ref Image canvasImage, Vector2 mousePos, out bool updateCanvas)
    {
        ...
        if (Raylib.IsMouseButtonDown(MouseButton.Left))
        {
            if (BrushSize == 1)
            {
                Color backgroundColor = RaylibClay.ToColor(Palettes.backgroundColor);
                Raylib.ImageDrawLineV(ref canvasImage, prevMousePos, mousePos, backgroundColor);
            }
            else
            {
    ...
    private void DrawThickLine(ref Image canvasImage, Vector2[] start, Vector2[] end)
    {
        ...
        for (int y = (int)minY; y <= (int)maxY; y++)
        {
            for (int x = (int)minX; x <= (int)maxX; x++)
            {
                ...
                {
                    Color backgroundColor = RaylibClay.ToColor(Palettes.backgroundColor);
                    Raylib.ImageDrawPixel(ref canvasImage, x, y, backgroundColor);
                    currentArea++;
                }
            }
        }
